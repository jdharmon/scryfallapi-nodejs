 ...
 *                      {stream} [response] - The HTTP Response stream if an error did not occur.
 */
function _getByArenaId(id, options, callback) {
   /* jshint validthis: true */
  let client = this.client;
  if(!callback && typeof options === 'function') {
    callback = options;
  ...
  // Construct URL
  let baseUrl = this.client.baseUri;
  let requestUrl = baseUrl + (baseUrl.endsWith('/') ? '' : '/') + 'cards/arena/{id}';
  requestUrl = requestUrl.replace('{id}', encodeURIComponent(id.toString()));
...
class Cards {
  ...
  constructor(client) {
    ...
    this._getByArenaId = _getByArenaId;
   ...
   * @reject {Error} - The error object.
   */
  getByArenaIdWithHttpOperationResponse(id, options) {
    let client = this.client;
    let self = this;
    return new Promise((resolve, reject) => {
      self._getByArenaId(id, options, (err, result, request, response) => {
        let httpOperationResponse = new msRest.HttpOperationResponse(request, response);
        httpOperationResponse.body = result;
        if (err) { reject(err); }
        else { resolve(httpOperationResponse); }
   ...
   *                      {stream} [response] - The HTTP Response stream if an error did not occur.
   */
  getByArenaId(id, options, optionalCallback) {
    let client = this.client;
    let self = this;
    if (!optionalCallback && typeof options === 'function') {
      optionalCallback = options;
    ...
    if (!optionalCallback) {
      ...
      return self._getByArenaId(id, options, optionalCallback);
    }
  }
